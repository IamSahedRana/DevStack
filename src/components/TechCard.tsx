...
export const TechCard: React.FC<{ tech: Technology }> = ({ tech }) => {
  const { isSelected, toggleStack } = useStack();
  const selected = isSelected(tech.id);

  // Determine if we should show the "Easy Start" badge based on difficulty
  const isBeginnerFriendly = tech.difficulty === "Beginner-Friendly";

  return (
    <div
      onClick={() => toggleStack(tech)}
      className={`relative flex flex-col p-6 rounded-2xl transition-all duration-300 border-2 cursor-pointer select-none ${
        selected
    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        <span
          className={`text-[11px] font-semibold px-2.5 py-1 rounded-full ${
            selected && isBeginnerFriendly ? 'bg-pink-50 text-pink-500' : getBadgeColor(tech.badge)
          }`}
        >
      ...
      <div className="flex items-center gap-3 text-[11px] text-gray-500 font-medium mb-5">
        <span className="bg-gray-50 text-gray-600 px-2.5 py-1 rounded-md border border-gray-100">{tech.category}</span>
        <span className="bg-gray-50 text-gray-600 px-2.5 py-1 rounded-md border border-gray-100">{tech.difficulty}</span>
      ...
      <div className="mt-auto">
        <button
          ...
          className={`w-full py-2.5 font-semibold text-sm rounded-xl transition-colors flex items-center justify-center gap-2 ${
            selected
              ? 'bg-[#db2777] hover:bg-[#be185d] text-white shadow-sm'
...
export default function TechGrid() {
  ...
  useEffect(() => {
    // Assuming technologies.json is in your public folder
  ...
  if (loading) {
    return (
      <div className="flex-1 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 w-full animate-pulse">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="h-72 bg-gray-50 rounded-2xl border border-gray-100" />
        ))}
      </div>
    );
  }
